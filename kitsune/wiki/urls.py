...
document_patterns = [
    ...
    url(r'^edit/(?P<revision_id>\d+)$', views.edit_document,
        name='wiki.new_revision_based_on'),
    url(r'^review/(?P<revision_id>\d+)$', views.review_revision,
    ...
    url(r'^show_translations$', views.show_translations,
    ...
    url(r'^get-votes-async', views.get_helpful_votes_async,
    ...
    # Delete a revision
    url(r'^revision/(?P<revision_id>\d+)/delete$', views.delete_revision,
    ...
    # Manage contributors
    url(r'^add-contributor$', views.add_contributor,
        name='wiki.add_contributor'),
    url(r'^remove-contributor/(?P<user_id>\d+)$', views.remove_contributor,
...
locale_patterns = [
    ...
    url(r'add-leader$', locale_views.add_to_locale,
        {'role': LEADER}, name='wiki.add_locale_leader'),
    url(r'^remove-leader/(?P<user_id>\d+)$', locale_views.remove_from_locale,
    ...
    url(r'^remove-editor/(?P<user_id>\d+)$', locale_views.remove_from_locale,
...
urlpatterns = [
    ...
    url(r'^watch-ready-for-review(?:/(?P<product>[^\/]+))?$',
        views.watch_locale, name='wiki.locale_watch'),
    url(r'^unwatch-ready-for-review(?:/(?P<product>[^\/]+))?$',
        views.unwatch_locale, name='wiki.locale_unwatch'),

    # (Un)subscribe to 'approved' notifications.
    url(r'^watch-approved(?:/(?P<product>[^\/]+))?$', views.watch_approved,
        name='wiki.approved_watch'),
    url(r'^unwatch-approved(?:/(?P<product>[^\/]+))?$', views.unwatch_approved,
        name='wiki.approved_unwatch'),

    # (Un)subscribe to 'ready-for-l10n' notifications.
    url(r'^watch-ready(?:/(?P<product>[^\/]+))?$', views.watch_ready,
        name='wiki.ready_watch'),
    url(r'^unwatch-ready(?:/(?P<product>[^\/]+))?$', views.unwatch_ready,
        name='wiki.ready_unwatch'),

    # Unhelfpul vote survey
    url(r'^unhelpful-survey', views.unhelpful_survey,
...
urlpatterns += [
    ...
    url(r'^all/discussions$', kbforums_views.locale_discussions,
        name='wiki.locale_discussions'),

    url(r'^discuss/watch_locale$', kbforums_views.watch_locale,
